...
CREATE  TABLE IF NOT EXISTS `inb201`.`Events_T` (
	...
	`CityName` VARCHAR(20) NOT NULL ,
    	`Event_Date` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,-- 'YYYY-MM-DD HH:MM:SS' format 
   	`Notes` VARCHAR(125), 
    	`Event_updated_Date` DATETIME DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`Event_ID`, `Event_Date`),
	UNIQUE(	`CityName`, `Event_Date`, `EventCode`),
...
CREATE  TABLE IF NOT EXISTS `inb201`.`Incidents` (
  ...
  `Finalised_date_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
